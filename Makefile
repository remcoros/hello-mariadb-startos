...
docker-images/aarch64.tar: manifest.yaml Dockerfile docker_entrypoint.sh $(APP_FILES)
ifeq ($(ARCH),x86_64)
else
...
docker-images/x86_64.tar: manifest.yaml Dockerfile docker_entrypoint.sh $(APP_FILES)
ifeq ($(ARCH),aarch64)
else
	mkdir -p docker-images
	docker buildx build --tag start9/$(PKG_ID)/main:$(PKG_VERSION) \
